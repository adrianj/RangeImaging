...
proc get_tccs { mem_if_memtype dqs_list period } {
	...
	switch -glob $f {
		...
		"*HSTL*/VPAD/Stratix III/2"  { return [list 277 277] }
		"*HSTL*/VPAD/Stratix III/3"  { return [list 323 323] }
		"*HSTL*/VPAD/Stratix III/4*"  { return [list 388 388] }
		...
		"*HSTL*/HPAD/Stratix III/2"  { return [list 277 277] }
		"*HSTL*/HPAD/Stratix III/3"  { return [list 333 333] }
		"*HSTL*/HPAD/Stratix III/4*"  { return [list 416 416] }
...
proc get_qdr_tsw_derating { dqs_list } { 
	...
	if {[catch {get_io_standard_node_delay -dst TSU -io_standard $io_std -parameters [list IO $interface_type PSEUDOX36]} tsw_setup] != 0 || $tsw_setup == "" || $tsw_setup == 0 || \
		...
		puts "Using Default Timing Model for derated tSW of $io_std $interface_type"
		set speed_grade $TimeQuestInfo(speed_grade)
		set family $TimeQuestInfo(family)
		set f "$interface_type/$family/$speed_grade"
		switch -glob $f {
			"VPAD/Stratix III/2"  { set result [list 50 250] }
			"VPAD/Stratix III/3"  { set result [list 75 275] }
			"VPAD/Stratix III/4*"  { set result [list 100 300] }

			"HPAD/Stratix III/2"  { set result [list 75 275] }
			"HPAD/Stratix III/3"  { set result [list 100 300] }
			"HPAD/Stratix III/4*"  { set result [list 125 325] }
			default { error "Can't recognise interface_type/family/speed grade combination $f"}
		}
	} else {
		puts "Using Quartus DDR Timing Model for derated tSW $tsw_setup $tsw_hold"
...
proc get_qdr_tccs_derating { dqs_list } { 
	...
	if {[catch {get_io_standard_node_delay -dst TCCS_LEAD -io_standard $io_std -parameters [list IO $interface_type PSEUDOX36]} tccs_lead] != 0 || $tccs_lead == "" || $tccs_lead == 0 || \
		...
		puts "Using Default Timing Model for derated tCCS of $io_std $interface_type"
		set result [list 20 20]
	} else {
		puts "Using Quartus DDR Timing Model for derated tCCS $tccs_lead $tccs_lag"
...
proc get_io_clock_divider_clocks {instname} {
  set io_clock_divider_pattern ${instname}_alt_mem_phy_siii_*inst|*|clkout
...
set corename "ddr2_devb_phy"
array unset pins_t
set dqsgroups 1
for {set i 0} {$i < $dqsgroups} {incr i} {
set dqs ${instname}_alt_mem_phy_siii_*inst|dpio|dqs_group\[$i\].dqs_op_gen.dqs_op|obuf|o
set dq ${instname}_alt_mem_phy_siii_*inst|dpio|dqs_group\[$i\].dq\[*\].dq_pad|gen_ddr_obuf.dq_obuf_inst|o
set dm  ${instname}_alt_mem_phy_siii_*inst|dpio|dqs_group\[$i\].dm_gen.dm_pad_gen\[*\].dm_pad|dq_obuf_inst|o
set dqsn ${instname}_alt_mem_phy_siii_*inst|dpio|dqs_group\[$i\].dqs_op_gen.dqsn_op_gen.dqsn_op|obuf|o
...
set patterns [list]
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_siii_*inst|*adc|*addr\[*\].addr_struct|*addr_pin|dataout
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_siii_*inst|*adc|*ba\[*\].ba_struct|*addr_pin|dataout
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_siii_*inst|*adc|*cas_n_struct|*addr_pin|dataout
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_siii_*inst|*adc|*ras_n_struct|*addr_pin|dataout
lappend patterns addrcmd_2t ${instname}_alt_mem_phy_siii_*inst|*adc|*we_n_struct|*addr_pin|dataout
lappend patterns addrcmd ${instname}_alt_mem_phy_siii_*inst|*adc|*cke\[*\].cke_struct|*addr_pin|dataout
lappend patterns addrcmd ${instname}_alt_mem_phy_siii_*inst|*adc|*odt\[*\].odt_struct|*addr_pin|dataout
lappend patterns addrcmd ${instname}_alt_mem_phy_siii_*inst|*adc|*cs_n\[*\].cs_n_struct|*addr_pin|dataout
set pattern ${instname}_alt_mem_phy_siii_*inst|clk|DDR_CLK_OUT\[*\].mem_clk_*obuf*
foreach_in_collection c [get_pins -compatibility_mode $pattern] {
  ...
  if {[llength $ports] == 0} {
    ...
    if { [string match ${instname}_alt_mem_phy_siii_*inst|clk|*mem_clk_n* $name] } {
...
set outputFileName "ddr2_devb_phy_autodetectedpins.tcl"
...
set ddr2_devb_phy_use_high_performance_timing 1
if {[namespace which -variable ::override_ddr2_devb_phy_use_high_performance_timing] != ""} {
  set ddr2_devb_phy_use_high_performance_timing $::override_ddr2_devb_phy_use_high_performance_timing
}
