...
foreach opc $opcs {
  ...
  if {$ddr2_devb_phy_use_high_performance_timing} {
    set assumptions_valid [verify_high_performance_timing_assumptions $instname pins]
  }
  # endif !ddr2_devb_phy_use_high_performance_timing
if {!$ddr2_devb_phy_use_high_performance_timing} {
# Write
...
set DQS_clock_period_jitter [expr [get_integer_node_delay -integer 2 -parameters {IO MAX HIGH} -src DQS_JITTER]/1000.0]
set DQS_clock_skew 0.035
set pll_step_size [expr $period / 40]
...
set su   [round_3dp [expr {$period*$dqs_phase/360.0 - [lindex $tsw 0]/1000.0 - $tDQSQ - $board_skew}]]
set hold [round_3dp [expr {$period*(0.5 - $tDCD) - $tQHS - $period*$dqs_phase/360.0 - [lindex $tsw 1]/1000.0 - $board_skew}]]
